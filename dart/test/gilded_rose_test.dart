...
main() async {
  ...
  group('Hammer', () {
    ...
    testItem(name, 'before sell date', 10, 40, 10, 40);
    testItem(name, 'on sell date', 0, 40, 0, 40);
    testItem(name, 'after sell date', -10, 40, -10, 40);
  });
  ...
  group('Concert Tickets', () {
    ...
    testItem(
        name, 'close to expired upper bound at max quality', 10, 50, 9, 50);
    testItem(
        name, 'close to expired upper bound near max quality', 10, 49, 9, 50);
    testItem(name, 'close to expired lower bound', 6, 10, 5, 12);
    testItem(name, 'close to expired lower bound at max quality', 6, 50, 5, 50);
    testItem(
        name, 'close to expired lower bound near max quality', 6, 49, 5, 50);
    testItem(name, 'very close to expired upper bound', 5, 10, 4, 13);
    testItem(
        name, 'very close to expired upper bound at max quality', 5, 50, 4, 50);
    testItem(name, 'very close to expired upper bound near max quality', 5, 48,
        4, 50);
    testItem(name, 'very close to expired lower bound', 1, 10, 0, 13);
    testItem(
        name, 'very close to expired lower bound at max quality', 1, 50, 0, 50);
    testItem(name, 'very close to expired lower bound near max quality', 1, 48,
        0, 50);
